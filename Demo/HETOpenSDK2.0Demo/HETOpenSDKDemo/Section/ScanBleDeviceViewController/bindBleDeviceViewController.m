...
@interface bindBleDeviceViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation bindBleDeviceViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cbp.state!=CBPeripheralStateConnected)
    {
        ...
        [self.bleBusiness bindBleDeviceWithPeripheral:cbp macAddress:nil completionHandler:^(NSString *deviceId, NSError *error) {
            ...
            if(error)
            {
               [HETCommonHelp showAutoDissmissAlertView:nil msg:@"绑定失败"];
               [weakself.navigationController popViewControllerAnimated:YES];
            }
            else
            {
                [HETCommonHelp showAutoDissmissAlertView:nil msg:@"绑定成功"];
