...
-(void)upgradeBtnAction
{
    ...
    [HETDeviceUpgradeBusiness deviceUpgradeCheckWithDeviceId:deviceId success:^(HETDeviceVersionModel * deviceVersionModel) {
        self.deviceVersionModel=deviceVersionModel;
        //NSLog(@"%@",dictValue);
        if(deviceVersionModel.newDeviceVersion){ // ([[dictValue allKeys] containsObject:@"newDeviceVersion"]) {
        
            self.oldDeviceVersion = deviceVersionModel.oldDeviceVersion;//dictValue[@"oldDeviceVersion"];
            self.devNewVersion =deviceVersionModel.newDeviceVersion;// dictValue[@"newDeviceVersion"];
            self.bleBinUrl =deviceVersionModel.filePath;// dictValue[@"filePath"];
            self.deviceVersionId =deviceVersionModel.deviceVersionId.integerValue;// [dictValue[@"deviceVersionId"] integerValue];
            self.bleDeviceVersionId =deviceVersionModel.deviceBleFirmId.integerValue;// [dictValue[@"deviceBleFirmId"] integerValue];
...
-(void)runBtnAction
{
    ...
    [HETDeviceRequestBusiness fetchDeviceRundataListWithDeviceId:deviceId startDate:lastlocationString endDate:nil pageRows:nil pageIndex:nil success:^(id responseObject) {
...
{
     NSString *deviceId=self.hetDeviceModel.deviceId;
    [HETDeviceRequestBusiness updateDeviceInfoWithDeviceId:deviceId deviceName:@"123fsdg" roomId:@"12" success:^(id responseObject) {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (self.oldDeviceVersion && self.devNewVersion && ![self.devNewVersion isEqualToString:self.oldDeviceVersion]){
        if (self.hetDeviceModel) {
            ...
            wifiUpgrade.deviceVersionModel=self.deviceVersionModel;
