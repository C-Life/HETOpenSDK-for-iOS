              ...
              deviceCfgDataFail:(void(^)(NSError *error))cfgDataFailBlock
{
    ...
    if(self)
    {
        ...
        _business=[[HETDeviceControlBusiness alloc]initWithHetDeviceModel:(HETDevice *)device deviceRunData:^(id responseObject) {
            NSLog(@"----------运行数据:%@",responseObject);
            //----这里结果根据自己设备在开放平台录入的运行数据协议的字段来处理结果-----//
            AromaDiffuserDeviceRunModel *model=[AromaDiffuserDeviceRunModel yy_modelWithJSON:responseObject];//[[AromaDiffuserDeviceRunModel alloc]initWithDic:responseObject];
            runDataSuccessBlock(model);
           //--------------------------------------------------------------//
        } deviceCfgData:^(id responseObject) {
            NSLog(@"配置数据:%@",responseObject);
            //----这里结果根据自己设备在开放平台录入的控制数据的协议的字段来处理结果-----//
            AromaDiffuserDeviceConfigModel *model=[AromaDiffuserDeviceConfigModel yy_modelWithJSON:responseObject];//[[AromaDiffuserDeviceConfigModel alloc]initWithDic:responseObject];
...
-(BOOL)isLittleLoop
{
    return NO;// [_business isLittleLoop];
}
...
- (void)deviceControlRequestWithModel:(AromaDiffuserDeviceConfigModel *)model withSuccessBlock:(void(^)(id responseObject))successBlock withFailBlock:(void(^)( NSError *error))failureBlock
{
    ...
    NSDictionary *responseObject=[model yy_modelToJSONObject];//[model convertModelToDic];
