...
-(void)viewWillAppear:(BOOL)animated
{
    _device=[[HETWIFIAromaDiffuserDevice alloc]initWithHetDeviceModel:self.hetDeviceModel deviceRunDataSuccess:^(AromaDiffuserDeviceRunModel *model) {
        ...
        NSLog(@"%@",model);
        NSDictionary *responseObject=[model yy_modelToJSONObject];
        NSString *jsonString=[self DataTOjsonString:responseObject];
        [self addText:jsonString];
        self.ignoreCount--;
        if(self.ignoreCount<0)
...
-(void)sendData
{
    ...
    AromaDiffuserDeviceConfigModel *model=[AromaDiffuserDeviceConfigModel yy_modelWithJSON:dic];//[[AromaDiffuserDeviceConfigModel alloc]initWithDic:dic];
...
-(UITextView *)logTextView
{
    if(!_logTextView)
    {
        UITextView *textView=[[UITextView alloc]initWithFrame:CGRectZero];//CGRectMake(0, _scrollV.frame.size.height+_scrollV.frame.origin.y,CCViewWidth, CCViewHeight-88*CC_scale-_scrollV.frame.size.height-_scrollV.frame.origin.y)];
